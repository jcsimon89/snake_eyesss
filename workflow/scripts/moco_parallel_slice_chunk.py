...
def find_missing_temp_files(chunk,fixed_path,
    ...
    for slice in range(nslices):
        ...
        for current_file in natsort.natsorted(temp_save_path.iterdir()):
            ...
            if '.npy' in current_file.name and moving_path.name in current_file.name and '_slice{}_'.format(slice) in current_file.name:
                ...
                if start_index == index_tracker:
                    # Great!
                    pass
                else:
                    # in case more than one file (e.g. 1 & 2) are missing!
                    while start_index > index_tracker:
                        print('Missing files, start index: ' + repr(index_tracker))
                        slice_and_index_of_missing_files.append(tuple([slice,index_range]))
                        index_tracker+=chunk #
    ...
    slice_and_index_of_missing_files = dict.fromkeys(slice_and_index_of_missing_files)
    slice_and_index_of_missing_files = list(slice_and_index_of_missing_files.keys())
    ...
    if len(slice_and_index_of_missing_files) > 0:
        ...
        for i in range(len(slice_and_index_of_missing_files)):
            ...
            current_index_range = slice_and_index_of_missing_files[i]
            current_time_chunk = list(range(current_index_range[0],current_index_range[1]+1))
            print('Missing (slice,index_range) currently working on: (' + str(current_slice) + ',' + repr(current_index_range) + ')')
            moco_slice(current_time_chunk,
                                current_slice,
                                fixed_path,
                                moving_path,
                                functional_channel_paths,
...
def combine_temp_files(chunk,moving_path,
                       functional_channel_paths,
                       temp_save_path,
                       moving_output_path,
    ...
    for slice in range(nslices):
        ...
        for current_file in natsort.natsorted(temp_save_path.iterdir()):
            ...
            if '.npy' in current_file.name and moving_path.name in current_file.name and '_slice{}_'.format(slice) in current_file.name:
                ...
                start_index = index_range[0]
                stitched_anatomy_brain[:,:,slice,current_time_chunk] = np.load(current_file)
                # Just a sanity check! E.g. for first image we expect '0'
                if index_tracker != start_index:
                    print('There seems to be a problem with the temp files for: ' + repr(current_file))
                    print('Previous start index was ' + str(index_tracker - chunk))
                    print('Current slice is {}'.format(slice))
                    print('Next index range (based on filename) was ' + repr(index_range))
        ...
        for slice in range(nslices):
            ...
            for current_file in natsort.natsorted(temp_save_path.iterdir()):
                    ...
                    if '.npy' in current_file.name and functional_path_one.name in current_file.name and '_slice{}_'.format(slice) in current_file.name:
                        ...
                        current_time_chunk = list(range(index_range[0],index_range[1]+1))
                        start_index = index_range[0]
                        stitched_functional_one[:,:,slice,current_time_chunk] = np.load(current_file)
                        # Just a sanity check! E.g. for first image we expect '0'
                        if index_tracker != start_index:
            ...
            for slice in range(nslices):
                ...
                for current_file in natsort.natsorted(temp_save_path.iterdir()):
                    ...
                    if '.npy' in current_file.name and functional_path_two.name in current_file.name and '_slice{}_'.format(slice) in current_file.name:
                        ...
                        current_time_chunk = list(range(index_range[0],index_range[1]+1))
                        start_index = index_range[0]
                        stitched_functional_two[:, :, slice, current_time_chunk] = np.load(current_file)
                        # Just a sanity check! E.g. for first image we expect '0'
                        if index_tracker != start_index:
                            print('There seems to be a problem with the temp files for: ' + str(current_file))
                            print('Previous start index was ' + str(index_tracker - chunk))
...
if __name__ == '__main__':
    ...
    print('Took: ' + repr(time.time() - time_start_moco) + 's to motion correct files')
    print('Motion correction done. Checking for missing files now')

    find_missing_temp_files(chunk,fixed_path,
                            moving_path,
                            functional_channel_paths,
                            temp_save_path,
                            fly_directory
    ...
    combine_temp_files(chunk,moving_path, functional_channel_paths, temp_save_path,
